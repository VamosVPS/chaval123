            ...
            if key in button_states:
                ...
                if button_states[key]:  # Si se selecciona
                    if key == "button_akdios":
                        ...
                        selected_buttons.append(Button.url("⚡ OKARUN_7", "https://t.me/OKARUN_7"))
...
async def update_buttons_ui(callback_event, button_states):
    buttons = [
        ...
        [
            Button.inline(f"Botón a @OKARUN_7{' (Seleccionado)' if button_states['button_okarun'] else ''}", b"button_okarun")
        ],
        [
...
async def me(event):
    ...
    coins = "∞" if has_premium else user_data.get("coins", 0)
    warnings = user_data.get("warnings", 0)
    role = user_data.get("role", "NO CLIENTE")
    queries = user_data.get("queries", 0)
    joined = user_data.get("joined", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))

    # Calcular días restantes del plan premium
    days_remaining = "N/A"
    if premium_end and parse_date(premium_end):
        days_remaining = (parse_date(premium_end) - datetime.now()).days
        days_remaining = max(days_remaining, 0)  # Evitar números negativos

    # Construir el perfil del usuario
    user_profile = f"""
    🆔 𝗨𝗦𝗘𝗥𝗡𝗔𝗠𝗘: @{username}
    🔑 𝗥𝗘𝗚𝗜𝗦𝗧𝗥𝗔𝗗𝗢: {'✅' if user_data['registered'] else '❌'}
    ⭐️ 𝗣𝗥𝗘𝗠𝗜𝗨𝗠: {'✅' if has_premium else '❌'} ({days_remaining} días restantes)
    💰 𝗖𝗥𝗘𝗗𝗜𝗧𝗢𝗦: {coins}
    ⚠️ 𝗔𝗗𝗩𝗘𝗥𝗧𝗘𝗡𝗖𝗜𝗔𝗦: {warnings}
    👤 𝗥𝗢𝗟: {role}
    🔍 𝗖𝗢𝗡𝗦𝗨𝗟𝗧𝗔𝗦: {queries}
    📅 𝗨𝗡𝗜𝗗𝗢: {joined}
    """
...
# Lista de palabras clave que indican una respuesta fallida
...
async def handle_command(event):
    ...
    if not message_text.startswith('/'):
        return  # Ignorar mensajes que no sean comandos
    
    command = message_text.split()[0]
    
    # Verificar si el usuario está bloqueado globalmente
    if global_ban_status["global_ban"]:
        await event.reply("❌ El bot está baneado globalmente.")
        return
    ...
    # Verificar si el comando requiere créditos
    if command in command_costs:
        cost = command_costs[command]
        if not has_premium and user_data['coins'] < cost:
            await event.reply(
                "[⛔] 𝗡𝗢 𝗧𝗜𝗘𝗡𝗘𝗦 𝗦𝗨𝗙𝗜𝗖𝗜𝗘𝗡𝗧𝗘𝗦 𝗖𝗥𝗘𝗗𝗜𝗧𝗢𝗦. 𝗥𝗘𝗖𝗔𝗥𝗚𝗔 𝗖𝗢𝗡 𝗟𝗢𝗦 𝗦𝗘𝗟𝗟𝗘𝗥.",
                buttons=[
                    [Button.url("☀️ 𝗔𝗞𝗗𝗜𝗢𝗦", "https://t.me/AKdios")],
                    [Button.url("🌩 𝗢𝗞𝗔𝗥𝗨𝗡", "https://t.me/OKARUN_7")],
                ],
    ...
    try:
        ...
        response = await process_command(command, event)
        
        # Verificar si la respuesta indica fallo
        if any(keyword in response for keyword in failed_response_keywords):
            await event.reply("❌ No se encontró información.", parse_mode='markdown')
            return  # No activar antispam ni deducir créditos
        
        # Enviar la respuesta al usuario
        await send_final_response(event, response)
        
        # Deducir créditos si no tiene plan premium
        if not has_premium:
        ...
        await event.reply("⚠️ Hubo un error al procesar tu solicitud. Inténtalo de nuevo.", parse_mode='markdown')

# Función para enviar la respuesta final al usuario
async def send_final_response(event, response):
    if isinstance(response, str):  # Respuesta de texto
        await event.reply(response, parse_mode='markdown')
    elif isinstance(response, tuple):  # Respuesta con archivo adjunto
        text, media = response
        await client.send_file(event.chat_id, media, caption=text, parse_mode='markdown')
    else:
        await event.reply("⚠️ Respuesta no válida.", parse_mode='markdown')

# Función para manejar el antispam
async def handle_antispam(event, username):
    if username in cooldowns and cooldowns[username] == 0:
        return False  # Usuario exento del antispam
    ...
    if time_since_last_command < cooldown:
        ...
        return True  # Antispam activo
    
    last_command_valid_time[username] = current_time
    return False  # Antispam no activo
...
async def forward_response(event):
    ...
    replacements = {
        ...
        '[#LEDER_BOT] → 𝗣𝗟𝗔𝗖𝗔S [PREMIUM]': '🚗 | 𝗣𝗟𝗔𝗖𝗔𝗦\n━━━━━━━━━━━━━━━━━━',
        '[#LEDER_BOT] → CORREOS [PREMIUM]': '📩 | 𝗜𝗡𝗙𝗢𝗥𝗠𝗔𝗖𝗜𝗢𝗡 𝗖𝗢𝗥𝗥𝗘𝗢\n━━━━━━━━━━━━━━━━━━',
        'CORREO': '𝗖𝗢𝗥𝗥𝗘𝗢 ',
